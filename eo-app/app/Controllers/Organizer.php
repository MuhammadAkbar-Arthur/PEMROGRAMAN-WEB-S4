...
<div class="container mx-auto p-4 md:p-6 space-y-8">
    ...
    <div class="bg-white dark:bg-gray-900 shadow-md rounded-2xl p-6 border border-gray-100 dark:border-gray-800">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl md:text-2xl font-bold text-gray-800 dark:text-white">Recent Wishlist</h2>
        ...
        <div class="grid md:grid-cols-3 gap-6">
            <?php if(!empty($favorites)): ?>
                <?php foreach($favorites as $f): ?>
                    <div class="border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/40 rounded-xl overflow-hidden hover:shadow-md transition">
                        <?php if($f['image']): ?>
                            <img src="/uploads/<?= $f['image']; ?>" class="w-full h-40 object-cover">
                        <?php endif; ?>
                        <div class="p-4">
                            <h3 class="font-bold text-lg mb-2 text-gray-800 dark:text-white"><?= esc($f['title']); ?></h3>
                            <p class="text-gray-500 text-sm mb-1">📍 <?= esc($f['location']); ?></p>
                            <p class="text-gray-500 text-sm">📅 <?= esc($f['date']); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <p class="text-gray-400 dark:text-gray-500 text-sm py-4">Wishlist kamu masih kosong.</p>
            <?php endif; ?>
        </div>
    </div>
