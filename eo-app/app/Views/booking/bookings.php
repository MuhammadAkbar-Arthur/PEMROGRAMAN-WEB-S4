...
function confirmDelete(button)
{
    ...
    Swal.fire({
        ...
        if(result.isConfirmed) {
            form.submit(); // Submit form POST secara aman jika disetujui
        }
    });
...
<div class="container mx-auto p-4 md:p-6">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="/" class="bg-blue-500 hover:bg-blue-600 text-white px-5 py-3 rounded-xl shadow">
            ← Back Home
        </a>
    </div>
    ...
    <?php if(empty($bookings)): ?>
        <div class="bg-white dark:bg-gray-900 shadow rounded-2xl p-10 text-center">
            ...
            <a href="/" class="bg-green-500 hover:bg-green-600 text-white px-6 py-3 rounded-xl">
                Explore Event
            </a>
        </div>
    <?php else: ?>
                    ...
                    <td class="p-4">
                        <div class="flex gap-2 items-center">
                            <?php if($b['status'] == 'approved'): ?>
                                <a href="/ticket/<?= $b['id'] ?>" class="bg-blue-500 text-white px-3 py-1 rounded">
                                    Ticket
                                </a>
                            <?php else: ?>
                            ...
                            <form action="/booking/delete/<?= $b['id']; ?>" method="post" class="inline">
                                <?= csrf_field(); ?>
                                <button type="button" onclick="confirmDelete(this)" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition-colors">
                                    Cancel
                                </button>
                            </form>
                        </div>
...
<script>
Swal.fire({ icon: 'success', title: 'Berhasil 🎉', text: '<?= esc(session()->getFlashdata('success')); ?>', confirmButtonColor: '#2563eb' });
</script>
<?php endif; ?>
